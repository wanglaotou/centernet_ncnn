...
int Centerobj::init(std::string model_path)
{
	...
	// std::string param = model_path + "centernet_mbv2.param";
	// std::string bin= model_path + "centernet_mbv2.bin";
	// resnet18 backbone
	std::string param = model_path + "res18.param";
	std::string bin= model_path + "res18.bin";
...
int Centerobj::detect(ncnn::Mat & inblob, std::vector<ObjInfo>& objs, int resized_w, int resized_h, float scoreThresh, float nmsThresh)
{
	...
	// ex.extract("535", heatmap);
	// ex.extract("536", scale);
	// ex.extract("537", offset);

	// string sstr = "/home/mario/Projects/Gaotong/doubledata.txt" //定义文件路径
	// char buf[22];
    // ofstream fin("./voc.txt");

	// int fea_h = heatmap.h;
	// int fea_w = heatmap.w;
	// int fea_c = heatmap.c;
	// std::cout << fea_h << " " << fea_w << " " << fea_c << std::endl;		// 64 80 20
	// for (int m=0; m < fea_c; m++){
	// 	for (int n=0; n < fea_w; n++){
	// 		for(int t=0; t < fea_h; t++){
	// 			// std::cout << heatmap[m, n, t] << " " << std::endl;
	// 			sprintf(buf,"%lf ", heatmap[m, n, t]);
